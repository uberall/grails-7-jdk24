...
class LocationRestApiSpec extends Specification implements ControllerUnitTest<LocationController> {
    ...
    void "POST /api/v1/locations creates a new location and returns JSON"() {
    ...
    void "POST /api/v1/locations with validation errors returns 400"() {
    ...
    void "POST /api/v1/locations with blank name returns validation error"() {
        given: "Location with blank name"
        request.contentType = 'application/json'
        request.json = [
            name: "",
            address: "123 Test Street",
    ...
    void "POST /api/v1/locations with blank address returns validation error"() {
        given: "Location with blank address"
        request.contentType = 'application/json'
        request.json = [
            name: "Valid Name",
            address: "",
    ...
    void "POST /api/v1/locations with address too long returns validation error"() {
        given: "Location with address exceeding max length"
        request.contentType = 'application/json'
        request.json = [
            name: "Valid Name",
        ...
        when: "Making a POST request with invalid address"
        controller.save()
    ...
    void "GET /api/v1/locations returns empty list when no locations exist"() {
    ...
    void "GET /api/v1/locations returns list of locations as JSON"() {
        given: "Some existing locations"
        def location1 = new Location(name: "Location 1", address: "Address 1", valid: true)
        location1.save(flush: true)
        def location2 = new Location(name: "Location 2", address: "Address 2", valid: false)
        ...
        response.json instanceof List
        response.json.size() == 2
        response.json.find { it.name == "Location 1" && it.valid == true } != null
        response.json.find { it.name == "Location 2" && it.valid == false } != null
    }

    void "GET /api/v1/locations/{id} with non-existent ID returns 404"() {
    ...
    void "PUT /api/v1/locations/{id} with non-existent ID returns 404"() {
    ...
    void "PUT /api/v1/locations/{id} with validation errors returns 422"() {
        ...
        def unchangedLocation = Location.get(location.id)
        unchangedLocation.name == "Old Name"
        unchangedLocation.address == "Old Address"
        unchangedLocation.valid == false
    }

    void "DELETE /api/v1/locations/{id} with non-existent ID returns 404"() {
    ...
    void "DELETE /api/v1/locations/{id} with associated listings should handle cascade"() {
    ...
    void "GET /api/v1/locations/{id} returns specific location as JSON"() {
        given: "An existing location"
    ...
    void "PUT /api/v1/locations/{id} updates existing location"() {
        given: "An existing location"
        def location = new Location(name: "Old Name", address: "Old Address", valid: false)
        location.save(flush: true)
    ...
    void "DELETE /api/v1/locations/{id} deletes existing location"() {
        given: "An existing location"
